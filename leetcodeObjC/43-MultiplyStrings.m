...
-(NSString*) multiply: (NSString*) nums1: (NSString*) nums2
{
    ...
    NSMutableArray *tmpres = [[NSMutableArray alloc] initWithCapacity:len];
    for(NSInteger i = 0; i < len; i++){
        [tmpres replaceObjectAtIndex: i withObject: @(0)];
    }
    ...
    for(NSInteger i = 0; i < len1 + len2; i++){
        ...
        val += carry;
        [tmpres replaceObjectAtIndex: i withObject: @(carry)];
        carry = [tmpres[i] intValue] / 10;
        val = [tmpres[i] intValue];
        [tmpres replaceObjectAtIndex: i withObject: @(val % 10)];
